...
from initialization import i_state, i_action, raw_material_price, matrix, receptive
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, action_bound, replacement, memory_capacity=10000, gamma=0.9, lr_a=0.001,
                 lr_c=0.002, batch_size=1000):
...
def excute_action(industry_state, state, action, layer):
    ...
    for i in range(layer):
        ...
        for j in range(update_state[i][0]):
            ...
            if i == 0:
                ...
                sell_amount.append(update_action[i][1][j] + state[i][j][7])
            else:
                update_action[i][1][j] = np.min([update_action[i][1][j], pro])
                purc_amount.append(update_action[i][1][j] / update_state[i][3][j])
                sell_amount.append(state[i][j][7])
    ...
    while (consumption > 0) & (i < (current_firm_num - 1)):
        ...
        consumption = consumption - update_action[layer - 1][0][id_ascent[i]] * update_temp[layer - 1][1][
            id_ascent[i]]
    if consumption < 0:
        settle_matrix[id_ascent[i]] = (consumption + update_action[layer - 1][0][id_ascent[i]] *
    ...
    ave_up_price = [raw_material_price]
    deal_result = [[] for _ in range(layer-1)]
    for i in range(layer-1):
        up_price = 0
        for j in range(industry_state[i][0]):
            up_price += action[i][j][0]
            deal = 0
            for k in range(industry_state[i+1][0]):
                deal += result[i][k][j]
            deal_result[i].append(deal)
        ave_up_price.append(up_price/industry_state[i][0])
        deal_result.append([0 for _ in range(industry_state[layer-1][0])])
    print(result)
    alpha = 3
    beta = 3
    for i in range(layer):
        ...
        for k in range(update_state[j][0]):
            ...
            state[j][k][7] = update_temp[j][1][k] - deal_result[j][k]
            reward[k] += alpha * (action[j][k][0] - ave_up_price[j]) - beta * state[j][k][7]
        update_reward[j] = reward
    post_profit.reverse()
    reward = update_reward
    next_state = [[] for _ in range(layer)]
    for i in range(layer):
        for j in range(industry_state[i][0]):
            competitor_price = []
            ...
            if i == 0:
                ...
                ave_com_price = sum(competitor_price) / (industry_state[i][0] - 1)
                next_state[i].append(state[i][j][0:8] + [ave_com_price])
...
if __name__ == '__main__':
    ...
    for e in range(MAX_EPISODES):
        print('episode:', e)
        ddpg = []
        state = [[] for _ in range(layer)]
        start_action = [[] for _ in range(layer)]
        for i in range(layer):
            state_dim = 9
            if i == 0:
                ddpg.append(DDPG(state_dim - 1, action_dim, action_bound=[10, 500],
                                 replacement=REPLACEMENT, memory_capacity=MEMORY_CAPACITY))
            else:
                ddpg.append(DDPG(state_dim, action_dim, action_bound=[10+i, 50],
                                 replacement=REPLACEMENT, memory_capacity=MEMORY_CAPACITY))
            competitor_price = list(i_action[i][0])
            for j in range(i_state[i][0]):
                ...
                if i == 0:
                    state[i].append([i_state[i][1][j], i_state[i][2][j], i_state[i][3][j],
                                     ...
                                     i_state[i][7][j], inventory])
                    ave_up_price = raw_material_price
                else:
                    ave_com_price = (sum(competitor_price)-competitor_price[j]) / (i_state[i][0]-1)
                    state[i].append([i_state[i][1][j], i_state[i][2][j], i_state[i][3][j],
                                     i_state[i][4][j], i_state[i][5][j], i_state[i][6][j],
                                     i_state[i][7][j], inventory, ave_com_price])
                start_action[i].append([i_action[i][0][j], i_action[i][1][j]])
        start_reward, next_state, start_profit = excute_action(i_state, state, start_action, layer)
        for i in range(layer):
            for j in range(i_state[i][0]):
                ddpg[i].store_transition(state[i][j], start_action[i][j], start_reward[i][j], next_state[i][j])

        for t in range(1, MAX_EP_STEPS + 1):
            action = [[] for _ in range(layer)]
            ave_up_price = []
            for i in range(layer):
                up_price = 0
                for j in range(i_state[i][0]):
                    ...
                    if i > 0:
                        if action[i][j][0] < ave_up_price[i-1]:
                            action[i][j][0] = ave_up_price[i-1]
                    up_price += action[i][j][0]
                    # 执行action 进行企业产量的更新
                ave_up_price.append(up_price/i_state[i][0])
            reward, next_state, profit = excute_action(i_state, state, action, layer)
